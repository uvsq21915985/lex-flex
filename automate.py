...
def reconnait(a, mot):
    ...
    """
    operations = 100
    return reconnaissance(a, 0, mot, operations)

def reconnaissance(a, etat_actuel, mot_actuel, operat_max):
    if operat_max == 0:
        return False
        
    if len(mot_actuel) == 0 and etat_actuel in a.final:
        return True
        
    if len(mot_actuel) > 0 and (etat_actuel, mot_actuel[0]) in a.transition:
        transitions = a.transition[(etat_actuel, mot_actuel[0])]
        for etat_arrivee in transitions:
            return reconnaissance(a, etat_arrivee, mot_actuel[1:], operat_max - 1)

    if (etat_actuel, "E") in a.transition:
        epsilon_transitions = a.transition[(etat_actuel, "E")]
        for etat_arrivee in epsilon_transitions:
            if reconnaissance(a, etat_arrivee, mot_actuel, operat_max - 1):
                return True
    return False
